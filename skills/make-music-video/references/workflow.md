...
# or one approved projectM preset from a prepared external pinned build:
...
External provider branches require a ready pinned local runtime. Astrofox binds local project/plugin/asset hashes. projectM accepts the hash-approved `mvt-wave`, `study-soft-flow`, or `study-beat-petals` preset with `locked-single` policy, one matching preset asset per job, and replays the song from frame zero before each global range. Their manifests bind silent CFR results. `compose` revalidates those results and uses MVT composition for canonical audio and saved lyrics; omit both `--lyrics` and `--font` when captions are off. Bundle only ordered compositions from the same source/profile. A provider video alone is not an S11 variant. The S13 feasibility output and S14 requests are historical; create requests with the current catalog-bound identity. The checked `scripts/projectm_visual_study.py` workflow compares both approved presets against an existing three-range `soft-harm` C preview; its song input and outputs stay outside Git.
