...
For Astrofox bar styling or a silent Astrofox overlay on an existing built-in preview, use [Astrofox variants](references/astrofox-variants.md). Its scripts expose palette, reflection, frequency range, bar geometry and overlay placement/intensity as saved parameters. Check the effective inherited defaults and compare the same ranges/audio/profile before selecting a look; song-specific colors are examples, not Skill defaults.
