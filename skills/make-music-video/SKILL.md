...
For an Astrofox visual variant, check `mvt doctor` and the local pinned checkout first. Create a typed provider request binding the canonical WAV, project, local plugins/assets, profile and one global range. Run `mvt provider astrofox`, then `mvt provider compose` with the project, timeline and optional saved lyrics plus checked font. Repeat per range, use `mvt provider bundle` on the ordered composition records, and compare that completed aggregate with a distinct built-in MVT preview. Keep all song files and output outside this repository. The provider video is silent; MVT supplies canonical audio and captions. Rendering must be offline, with no remote/unhashed plugins. A failed or unproven provider job is not a completed variant. The Astrofox editor may be opened only to debug a failure. projectM and MCP are not available production routes.
