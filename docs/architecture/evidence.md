...
- [Astrofox source, revision 126403958e5644a6fbb91d6623626474dd199205](https://github.com/astrofox-io/astrofox/tree/126403958e5644a6fbb91d6623626474dd199205): `docs/plugin-authoring.md` specifies deterministic export frame time/delta/seed and local plugin identity; `docs/desktop-capabilities.md` documents offline desktop frames through an Electron FFmpeg bridge. `VideoExporter.export()` stops the live loop, calls `renderer.renderFrame(frame, fps)`, writes RGBA through the preload pipe bridge and can omit audio. The project loader and export entry remain renderer-internal and no standalone headless render CLI or command-line job protocol is present. S13 therefore pins this exact source and carries a tested downstream CLI patch instead of driving the UI.
- [projectM core](https://github.com/projectM-visualizer/projectm) is an LGPL libprojectM library that accepts PCM and renders through OpenGL; it does not capture audio or ship presets. The related [gst-projectm](https://github.com/projectM-visualizer/gst-projectm) documents offline file-to-video pipelines and preset-driven conversion, including macOS build guidance. Neither is installed on the observed host. S13 may prove a pinned short silent-CFR provider against the external-backend contract, but does not advertise a production projectM adapter.
