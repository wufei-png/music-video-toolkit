...
- [Astrofox source, revision 126403958e5644a6fbb91d6623626474dd199205](https://github.com/astrofox-io/astrofox/tree/126403958e5644a6fbb91d6623626474dd199205): the upstream renderer has deterministic export frame time/delta/seed and `VideoExporter.export()` can omit audio. Upstream has no standalone headless render CLI. S13 pins its pnpm lock and MIT license and adds two hash-bound downstream patches in an external checkout. Clean builds, hidden job loading, local hash-pinned plugins/assets, network denial, silent CFR exports, cancellation cleanup and same-environment repeat hashes were exercised without UI automation. The patched CLI is the supported provider route.
- [projectM core](https://github.com/projectM-visualizer/projectm) is an LGPL libprojectM library that accepts PCM and renders through OpenGL; it does not capture audio or ship presets. S13 pins core `1e7ef7803b69024d1e0656705670adda2ffac817`, builds it in an external environment and feeds canonical PCM to a small MIT-licensed feasibility provider with synthetic MIT preset/texture identities. Two six-frame 1920x1080/30 silent outputs conformed to the provider contract but differed bytewise. Nonzero global-time replay and the cause of pixel variation remain unproven. projectM has no production adapter or advertised capability.
...
| projectM determinism/global time | S13 six-frame feasibility conforms, repeat bytes differ | Keep unavailable until a separate adapter proves repeatable global-range behavior and production composition |
